...
if(isset($_POST['submit'])){
    ...
    if(!empty($name)){ //checks whether the variable is not empty
        $update_name = $conn->prepare("UPDATE `users` SET name = ? WHERE id = ?");
        $update_name->execute([$name, $user_id]);
        $success_msg[] = 'name updated!';
    }

    if(!empty($number)){ //checks whether the variable is not empty
    ...
    $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709'; //empty hashed '' string 
    $prev_pass = $fetch_account['password']; //getting the current password
    $old_pass = sha1($_POST['old_pass']); // asking the user for the old password
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="tel" name="name" maxlength="50" placeholder="<?= $fetch_account['name']; ?>" class="box">
      <input type="email" name="email" maxlength="50" placeholder="<?= $fetch_account['email']; ?>" class="box">
      <input type="number" name="number" min="0" max="9999999999" maxlength="10" placeholder="<?= $fetch_account['number']; ?>" class="box">
      ...
      <input type="submit" value="update now" name="submit" class="btn-search">
   </form>
