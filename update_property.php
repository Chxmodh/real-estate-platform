...
if(isset($_POST['update'])){
    ...
    $ownership = filter_var($ownership, FILTER_SANITIZE_STRING);
    $loan = $_POST['loan'];
    ...
    $image_01_ext = pathinfo($image_01, PATHINFO_EXTENSION);// getting the extension png/jpg etc...
    $rename_image_01 = create_unique_id().'.'.$image_01_ext; //joins the two unique id with the extension (abc123.png)
    ...
    $image_02_ext = pathinfo($image_02, PATHINFO_EXTENSION);// getting the extension png/jpg etc...
    $rename_image_02 = create_unique_id().'.'.$image_02_ext; //joins the two unique id with the extension (abc223.png)
    ...
    $image_03_ext = pathinfo($image_03, PATHINFO_EXTENSION);// getting the extension png/jpg etc...
    $rename_image_03 = create_unique_id().'.'.$image_03_ext; //joins the two unique id with the extension (abc323.png)
    ...
    $image_04_ext = pathinfo($image_04, PATHINFO_EXTENSION);// getting the extension png/jpg etc...
    $rename_image_04 = create_unique_id().'.'.$image_04_ext; //joins the two unique id with the extension (abc433.png)
    ...
    $image_05_ext = pathinfo($image_05, PATHINFO_EXTENSION);// getting the extension png/jpg etc...
    $rename_image_05 = create_unique_id().'.'.$image_05_ext; //joins the two unique id with the extension (abc523.png)
    ...
    $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, status = ?, furnished = ?, floor = ?, deposit = ?, bedroom = ?, bathroom = ?, balcony = ?, carpet = ?, age = ?, total_floors = ?, ownership = ?, loan = ?, lift = ?, security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
    $update_listing->execute([$property_name, $address, $price, $type, $offer, $status, $furnished, $floor, $deposite, $bedroom, $bathroom, $balcony, $carpet, $age, $total_floors, $ownership, $loan, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);
...
<section class="property-form">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="box">
            ...
            <input type="text" name="property_name" required maxlength="50" required placeholder="enter property name" class="input" 
        ...
        <div class="flex">
            ...
            <div class="box">
                ...
                <input type="number" name="deposit" required min="0" max="9999999999" maxlength="10" placeholder="enter deposit amount" class="input"
            ...
            <div class="box">
                ...
                <select name="floor" class="input" required>
                    <option value="<?= $fetch_property['floor']; ?>" selected> 
                    <?= $fetch_property['floor']; ?></option>
            ...
            <div class="box">
                ...
                placeholder="how floors available?" class="input" value="<?= $fetch_property['total_floors']; ?>">
            </div>
            <!-- <div class="box">
                <p>Property ownership <span>*</span></p>
                <input type="number" name="ownership" required min="0" max="99" maxlength="2" 
                placeholder="property floor number" class="input" value="<?= $fetch_property['ownership']; ?>">
            </div> -->

            <div class="box">
                <p>Property ownership <span>*</span></p>
                <select name="ownership" required class="input">
                    <option value="<?= $fetch_property['ownership']; ?>" selected> 
                    <?= $fetch_property['ownership']; ?></option>
                    <option value="freehold ownership">Freehold ownership</option>
                    <option value="leasehold ownership">Leasehold ownership</option>
        ...
        <div class="flex"> 
            <div class="box">
                ...
                onclick="return confirm('delete image 02?');"> <!-- edit confirm message-->
                <?php }; ?>
                <p>update image 02</p>
                <input type="file" name="image_02" class="input" accept="image/*"> <!-- accepts only images-->
            </div>
            ...
            <div class="box">
                ...
                onclick="return confirm('delete image 03?');"> <!-- class inline btn can be change to btn-->
                <?php }; ?>
                <p>update image 03</p>
            ...
            <div class="box">
                ...
                onclick="return confirm('delete image 04?');"> <!-- edit confirm message-->
                <?php }; ?>
                <p>update image 04</p>
            ...
            <div class="box">
                ...
                onclick="return confirm('delete image 05?');"> <!-- edit confirm message-->
                <?php }; ?>
                <p>update image 05</p>
        ...
        <input type="submit" value="update property" class="btn-post" name="update">
    </form>

    <?php
        }
    }else{
        echo'<p class="empty">no property was found!</p>';
    }
    ?>
